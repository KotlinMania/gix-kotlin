# Claude Code Project Instructions — gix-kotlin

## Project Overview

This is **gix-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`gix`](https://crates.io/crates/gix).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/gix/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/gix>
